...
export default function LetSuspense ({
	...
	useEffect(
		...
		[ condition, children, checkOnce, initialDelay, isChecked, multiplier ]
	);
	return <Fragment>{component.map((component, index) => <Fragment key={index}>{component} </Fragment>)}</Fragment>;
}
